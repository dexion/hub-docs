...
| `LLM_MODEL`                    | `glm-4-flash` | Модель (рекомендация для prod: `glm-4-plus` или эквивалент) |
| `LLM_DRY_RUN`                  | `false`       | Логировать диалог, не отправлять реальные запросы           |
| `LLM_FALSE_POSITIVE_THRESHOLD` | `0.85`        | Порог confidence для авто-пометки FP. 0.9 — безопаснее      |
| `LLM_WORKERS`                  | `3`           | **0 в backend, > 0 только в worker-pod**                    |
