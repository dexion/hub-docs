...
1. Браузер → `/login` → Hub редиректит в Keycloak (OIDC authorization code)
2. Юзер логинится в Keycloak, возвращается с `code`
3. Backend обменивает `code` на Keycloak tokens → создаёт/обновляет юзера в БД → выпускает внутренний JWT (access + refresh)
4. Все последующие запросы — с `Authorization: Bearer <access_token>`

## Безопасность

- **Keycloak** — единая точка аутентификации, поддерживаются realm-based роли
