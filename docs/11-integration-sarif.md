...
# 2. Create API Key
curl -X POST "https://hub.example.com/api/v1/service-accounts/$SA_ID" \
  -H "Authorization: Bearer $ADMIN_JWT" \
  -H "Content-Type: application/json" \
...
```json
{
  "data": {
    "id": "uuid",
    "key": "sshub_AAA...full-key-shown-once...ZZZ",
    "key_prefix": "sshub_AAA",
    "expires_at": "2026-12-02T..."
  }
}
```

`key_prefix` хранится в БД для аудита (можно отслеживать, какой ключ использовался). Полный ключ хешируется (SHA-256) и сравнивается при каждом запросе.
...
| -------------- | ------ | -------- | ------------------------------------------------------------------ |
| `file`         | file   | ✅       | SARIF-файл (можно gzip'ed с расширением `.sarif.gz`)               |
...
- name: Upload SARIF to Hub
  ...
  run: |
    curl -X POST "${{ secrets.HUB_URL }}/api/v1/products/${{ secrets.HUB_PRODUCT_ID }}/reports" \
      -H "Authorization: Bearer ${{ secrets.HUB_API_KEY }}" \
