...
| `SERVER_PORT`       | int                                   | `8080` (в compose задан 8082) | Порт backend API                                                                        |
...
| Переменная    | Тип      | Default                                          | Описание        |
| ------------- | -------- | ------------------------------------------------ | --------------- |
| `DB_HOST`     | hostname | `localhost` (в compose: `sshub-postgres`)        | PostgreSQL host |
...
| `KC_AUDIENCES`                     | CSV strings | —              | Разрешённые значения `aud` (пустой список по умолчанию). **Не использовать `account` в production** |
| `KC_AUTO_PROVISION`                | bool        | `false`        | Создавать ли пользователя автоматически при первом валидном JWT (включается только явным `=true`) |
...
| `LLM_MODEL`                    | string | `glm-4-plus`  | Модель                                           |
| `LLM_DRY_RUN`                  | bool   | `false`       | Без отправки реальных запросов                   |
| `LLM_FALSE_POSITIVE_THRESHOLD` | float  | `0.9`         | Минимальная уверенность для пометки FP           |
| `LLM_WORKERS`                  | int    | `5`           | Воркеры. **0 в backend, >0 только в worker-pod** |
...
Уровень и формат zap-логов **не настраиваются отдельными переменными** — они производны от `APP_ENV`:

- `APP_ENV=development` → console-формат, уровень `Debug`
- любое другое значение (`production`, `test`, …) → JSON-формат, уровень `Info`

Отдельных `LOG_LEVEL` / `LOG_FORMAT` в коде нет. Ротация файловых логов управляется переменными `LOG_MAX_SIZE_MB`, `LOG_MAX_BACKUPS`, `LOG_MAX_AGE_DAYS`, `LOG_COMPRESS`, `LOGS_PATH`.
...
# Kubernetes
kubectl -n hub get deploy hub-backend -o yaml | grep -A2 'env:'
